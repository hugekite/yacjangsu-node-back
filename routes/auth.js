...
router.post("/kakaoJoinProcess", function (request, response) {
  ...
  db.query(sql.kakao_check, [kakao.user_id], function (error, results, fields) {
    if (results.length <= 0) {
      db.query(
        sql.kakaoJoin,
        [kakao.user_id, kakao.user_name, kakao.user_id],
  ...
  db.query(
    ...
    function (error, results, fields) {
      ...
      if (results.length > 0) {
        ...
        db.query(
          sql.naverlogin,
          [naverlogin.email, naverlogin.id, naverlogin.nickname, null],
          function (error, result) {
            if (error) {
              console.error(error);
              return response.status(500).json({ error: "error" });
...
// 관리자 회원 검색 리스트
router.get(
  "/admin/userlist/:sortCase/:keyword/:page/:pagesize",
...
router.post("/event_o", function (request, response) {
  ...
  db.query(
    sql.event_o,
    [user.user_doevent, user.user_no],
    function (error, results, fields) {
      if (error) {
        console.error(error);
        return response.status(500).json({ error: "회원리스트에러" });
...
router.post("/like_addCart", function (request, response) {
  ...
  db.query(
    sql.like_addCart,
    [userNo, likeCart.goodsNo],
    function (error, results, fields) {
      if (error) {
        console.error(error);
        return response
          .status(500)
          .json({ error: error.message, userNo, likeCart });
      }
      response.json(results);
    }
  );
